...
namespace FikaAmazonAPI.SampleCode
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            var mpId = config.GetSection("FikaAmazonAPI:MarketPlace").Value;
            AmazonConnection amazonConnection = new AmazonConnection(new AmazonCredential()
            {
                ...
                MarketPlace = MarketPlace.GetMarketPlaceByID(mpId),
            });

            var shipments =new FulFillmentInboundSample(amazonConnection).GetInboundShipments();
